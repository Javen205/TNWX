...
export class ApiConfigKit {
	...
	public static get getToken(): string {
		...
		if (!token) {
			throw new Error(
				'需事先调用 ApiConfigKit.putApiConfig(apiConfig) 将 appId 对应的 ApiConfig 对象存入后, 才可以使用 ApiConfigKit.getToken 系列方法'
			);
		}
		return token;
	}
	...
	public static getApiConfigByAppId(appId: string): ApiConfig {
		...
		if (!cfg) {
			throw new Error(
				'需事先调用 ApiConfigKit.putApiConfig(apiConfig) 将 appId 对应的 ApiConfig 对象存入后,才可以使用 ApiConfigKit.getApiConfig 系列方法'
			);
		}
		return cfg;
	}
